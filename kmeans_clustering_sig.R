...
rep_mean = function(x){
	xm = c((x[1]+x[2])/2, (x[3]+x[4])/2, (x[5]+x[6])/2, (x[7]+x[8]+x[9])/3, )	
	return(xm)
}
...
kmeans_dist = c()
ave_fc_col_var = sum(apply(rep_mean, 2, var))

for (i in c(1:20)){
print(i)
set.seed(2018)
#dr = as.matrix(rep_mean_fc)
dr = as.matrix(rep_mean)
fit = kmeans(dr, i)
...
for (i in plot_cluster_rank){
	...
	write.table(pk_k, paste('kmean.TS.', toString(k), '.pk.rawsig4.txt', sep=''), quote=F, col.names=F, row.names=F, sep='\t')
	### plot boxplot
	pdf(paste('kmeans.box.nodrb.rawsig4.', toString(k), '.pdf', sep=''))
...
pdf(paste('kmean.TS.nodrb.rawsig4.', toString(nr), '.pdf', sep=''))
...
pdf(paste('kmean.TS.nodrb.label.rawsig4.', toString(nr), '.pdf', sep=''))
...
png(paste('kmean.TS.nodrb.label.rawsig4.', toString(nr), '.png', sep=''))
pheatmap(dr_kmeans_plot_label, cluster_rows = FALSE, cluster_cols = FALSE,annotation_names_row=FALSE,annotation_names_col=FALSE,show_rownames=FALSE,show_colnames=FALSE)
dev.off()

colnames(pk_k_all)[length(colnames(pk_k_all))] = 'cluster_id'
write.table(pk_k_all, paste('kmean.TS.all.pk.rawsig4.txt', sep=''), quote=F, col.names=T, row.names=F, sep='\t')
