...
for (i in c(1:6)){
	...
	points(mean(d_qtpcr_mat_scale_i[peak_type!='Strong-Stable']), mean(d_chipseq_average_mat_FC_i[peak_type!='Strong-Stable']), pch=15, cex = 1.5, col='black')
	abline(0, 1, lwd=1.5, col = 'red')
	AB_1 = s3norm(d_qtpcr_mat_scale_i+1, d_chipseq_1_mat_FC_i+1, peak_type!='Strong-Dynamic', peak_type=='Strong-Dynamic', 1.0, 2.0, 1e-5)
	d_chipseq_1_mat_FC_i_s3norm = AB_1[1] * (d_chipseq_1_mat_FC_i ^ AB_1[2])
	AB_2 = s3norm(d_qtpcr_mat_scale_i+1, d_chipseq_2_mat_FC_i+1, peak_type!='Strong-Dynamic', peak_type=='Strong-Dynamic', 1.0, 2.0, 1e-5)
	d_chipseq_2_mat_FC_i_s3norm = AB_2[1] * (d_chipseq_2_mat_FC_i ^ AB_2[2])
	AB_average = s3norm(d_qtpcr_mat_scale_i+1, d_chipseq_average_mat_FC_i+1, peak_type!='Strong-Dynamic', peak_type=='Strong-Dynamic', 1.0, 2.0, 1e-5)
