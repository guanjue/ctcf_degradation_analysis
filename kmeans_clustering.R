...
for (i in plot_cluster_rank){
	...
	write.table(pk_k, paste('kmean.TS.', toString(k), '.pk.txt', sep=''), quote=F, col.names=F, row.names=F, sep='\t')
	### plot boxplot
	pdf(paste('kmeans.box.nodrb.', toString(k), '.pdf', sep=''))
...
pdf(paste('kmean.TS.nodrb.', toString(nr), '.pdf', sep=''))
...
pdf(paste('kmean.TS.nodrb.label.', toString(nr), '.pdf', sep=''))
...
png(paste('kmean.TS.nodrb.label.', toString(nr), '.png', sep=''))
pheatmap(dr_kmeans_plot_label, cluster_rows = FALSE, cluster_cols = FALSE,annotation_names_row=FALSE,annotation_names_col=FALSE,show_rownames=FALSE,show_colnames=FALSE)
dev.off()

colnames(pk_k_all)[length(colnames(pk_k_all))] = 'cluster_id'
write.table(pk_k_all, paste('kmean.TS.all.pk.txt', sep=''), quote=F, col.names=T, row.names=F, sep='\t')
