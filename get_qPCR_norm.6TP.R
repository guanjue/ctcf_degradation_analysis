...
qPCR_pk_id = read.table('qtPCR_sig.6TP.ids.txt', header=F)
...
for (i in c(1:length(IP_list))){
	print(IP_list[i])
	qPCR = scan(paste('qtPCR_sig.6TP.', toString(TP_list[i]), '.txt', sep=''))
	used_id = qPCR!=-10
	qPCR_expand = qPCR * 100
	for (j in c(1:length(method_list))){
		d_IP = scan(paste('qtPCR_regions_signal_tab/', toString(IP_list[i]), '.qtPCR_regions.sorted.signal.6TP.', method_list[j], '.tab', sep=''))
	...
	### get log2FC_MACS
	d_IP0 = scan(paste('qtPCR_regions_signal_tab/', toString(IP_list[i]), '.qtPCR_regions.sorted.signal.6TP.', 'fc', '.tab', sep=''))
	d_IP = log2(d_IP0+1)
	### get TSnorm scale factor
	sf_tmp = 1/mean(d_IP[used_id]) * mean(qPCR_expand[used_id])
	sf_s3_tmp = s3norm((qPCR_expand[used_id]), (d_IP[used_id]))
...
### get rownames
rownames(qPCR_expand_mat) = qPCR_pk_id[,1]
rownames(d_IP_raw_mat) = qPCR_pk_id[,1]
rownames(d_IP_ts_mat) = qPCR_pk_id[,1]
rownames(d_IP_s3_mat) = qPCR_pk_id[,1]
rownames(sf_mat) = 'SF'
...
### output
dir.create('qPCR_cor_6TP')

### plot correlation boxplot
pdf('qPCR_cor_6TP/correlation_boxplot_ts_s3.pdf')
...
for (i in c(1: dim(d_IP_ts_mat)[2])){
	output_fig_name = paste('qPCR_cor_6TP/', colnames(d_IP_ts_mat)[i], '.scatterplot.pdf', sep='')
...
get_barplot_fig = function(qPCR_expand_mat, method_name, k){
	...
	for (TPtmp in unique(TP_list)[c(1,2,3,4,6,5)]){
	...
	colnames(qPCR_expand_mat_TP) = unique(TP_list)[c(1,2,3,4,6,5)]
...
pdf('qPCR_cor_6TP/6tp_bar.qPCR_6TP.pdf', width=12, height=12)
par(mfrow=c(4,1))
...
text(cex=1, x=3.5+(c(1:dim(t(qPCR_expand_mat_TP))[2])-1)*7, y=-30, rownames(qPCR_expand_mat_TP), xpd=TRUE, srt=45)
box()
...
text(cex=1, x=3.5+(c(1:dim(t(d_IP_ts_mat_TP))[2])-1)*7, y=-30, rownames(d_IP_ts_mat_TP), xpd=TRUE, srt=45)
box()
...
text(cex=1, x=3.5+(c(1:dim(t(d_IP_s3_mat_TP))[2])-1)*7, y=-30, rownames(d_IP_s3_mat_TP), xpd=TRUE, srt=45)
box()
...
### write scale factors
write.table(sf_mat, 'TSnorm_SF.6TP.txt', col.names=TRUE, row.names=TRUE, quote=FALSE, sep='\t')
write.table(sf_lm_mat, 's3norm_SF.6TP.txt', col.names=TRUE, row.names=TRUE, quote=FALSE, sep='\t')


pdf('qPCR_cor_6TP/d_IP_s3_mat.pdf', width=12, height=7)
par(mfrow=c(1,2))
