...
BIC = mclustBIC(rep_mean_fc[,-1], modelNames=model_list)

pdf('bic.pdf')
plot(BIC)
dev.off()

fit = Mclust(rep_mean_fc[,-1], x = BIC)
...
for (i in plot_cluster_rank){
	...
	write.table(pk_k, paste('mclust.TS.', toString(k), '.pk.txt', sep=''), quote=F, col.names=F, row.names=F, sep='\t')
	### plot boxplot
	pdf(paste('mclust.box.', toString(k), '.pdf', sep=''))
...
pdf(paste('mclust.TS.nodrb.', toString(nr), '.pdf', sep=''))
...
pdf(paste('mclust.TS.nodrb.label.', toString(nr), '.pdf', sep=''))
...
png(paste('mclust.TS.nodrb.label.', toString(nr), '.png', sep=''))
pheatmap(dr_mclust_plot_label, cluster_rows = FALSE, cluster_cols = FALSE,annotation_names_row=FALSE,annotation_names_col=FALSE,show_rownames=FALSE,show_colnames=FALSE)
dev.off()

colnames(pk_k_all)[length(colnames(pk_k_all))] = 'cluster_id'
write.table(pk_k_all, paste('mclust.TS.all.pk.txt', sep=''), quote=F, col.names=T, row.names=F, sep='\t')
