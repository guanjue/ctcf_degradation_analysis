...
do
	...
	var2=$(echo "$LINE" | awk -F '\t' '{print $1}' | awk -F '_' -v OFS='_' '{print $2,$3,$4,$5}')
	echo $index
	#echo $var1
	#echo $var2
	...
	if (( line_num > 5000 )); then
		...
		echo 'use all'
		cat $index'.bed' | awk -F '\t' -v OFS='\t' -v l1="$var1" -v l2="$var2" '{print $1":"int(($2+$3)/2), l2";"l1}' >> all_cluster_s_5_ic.txt
	fi 
