...
sample_list_file = 'id_list_check.txt'
...
for (i in c(1:6)){
	...
	d_nonpk_1_mat_FC_i_ref = (d_nonpk_1_mat[,1]+smallnum)/(d_nonpk_input_mat[,1]+smallnum)
	d_nonpk_2_mat_FC_i_ref = (d_nonpk_2_mat[,1]+smallnum)/(d_nonpk_input_mat[,1]+smallnum)
	d_nonpk_average_mat_FC_i_ref = ((d_nonpk_1_mat[,1]+d_nonpk_2_mat[,1])/2+smallnum)/(d_nonpk_input_mat[,1]+smallnum)
	...
	points(mean(d_qtpcr_mat_scale_i[peak_type!='Strong-Stable']), mean(d_chipseq_average_mat_FC_i[peak_type!='Strong-Stable']), pch=15, cex = 1.5, col='black')
	abline(0, 1, lwd=1.5, col = 'red')
	used_pk = peak_type!='1Strong-Dynamic'
