...
do
	...
	time /storage/home/gzx103/group/software/ucsc/bigWigAverageOverBed 'bam_bw_bedgraph/'$id'IP.bw' allmerged.sorted.bed $id'.qtPCR_regions.sorted.5T7D.tab'
...
do
	...
	paste $id'.qtPCR_regions.sorted.signal.5T7D1.tab' $id'.qtPCR_regions.sorted.signal.5T7D.macsBG.tab' | awk -F '\t' -v OFS='\t' '{print ($1)/($2)}' > $id'.qtPCR_regions.sorted.signal.5T7D.fc.tab'
	rm $id'.qtPCR_regions.sorted.signal.5T7D1.tab' $id'.qtPCR_regions.sorted.signal.5T7D2.tab' 
...
do
	...
	paste $id'.qtPCR_regions.sorted.signal.6TP1.tab' $id'.qtPCR_regions.sorted.signal.6TP.macsBG.tab' | awk -F '\t' -v OFS='\t' '{print ($1)/($2)}' > $id'.qtPCR_regions.sorted.signal.6TP.fc.tab'
	rm $id'.qtPCR_regions.sorted.signal.6TP1.tab' $id'.qtPCR_regions.sorted.signal.6TP2.tab' 
